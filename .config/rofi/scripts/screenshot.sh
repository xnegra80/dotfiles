...
crtc() {
  notify-send "Screenshot" "Select a region to capture" -t 1000
  ...
  notify-send "Screenshot" "Region copied to Clipboard" -t 1000
}
...
crtf() {
  ...
  notify-send "Screenshot" "Region saved to $screenshot_directory" -t 1000
}
...
cstc() {
  ...
  xclip -selection clipboard -t image/png /tmp/screenshot_clip.png
  rm /tmp/screenshot_clip.png
  notify-send "Screenshot" "Screenshot copied to Clipboard" -t 1000
}
...
cstf() {
  ...
  notify-send "Screenshot" "Screenshot saved to $screenshot_directory" -t 1000
}

rgrtf() {
  notify-send "Screenshot" "Select a region to record" -t 1000
  dt=$(date '+%d-%m-%Y %H:%M:%S')
  ffcast -q $(slop -n -f '-g %g ' && countdown) rec /tmp/screenshot_gif.mp4
  notify-send "Screenshot" "Converting to gif... (this can take a while)" -t 1000
  video_to_gif /tmp/screenshot_gif.mp4 "$screenshot_directory/$dt.gif"
  rm /tmp/screenshot_gif.mp4
  notify-send "Screenshot" "Recording saved to $screenshot_directory" -t 1000
}
